...
export async function GET(request) {
  try {
    ...
    if (!isValidUuid(milestoneId)) {
      return NextResponse.json({ message: 'milestone_id wajib UUID yang valid.' }, { status: 400 });
    }
    ...
    if (error) {
      return NextResponse.json({ message: 'Gagal mengambil data payments', error: error.message }, { status: 500 });
    }

    return NextResponse.json({ message: 'Berhasil mengambil data payments', data }, { status: 200 });
  } catch (err) {
...
export async function POST(request) {
  try {
    ...
    if (!PAYMENT_STATUS.includes(status)) {
      return NextResponse.json({ message: `status harus salah satu: ${PAYMENT_STATUS.join(', ')}` }, { status: 400 });
    }
    ...
    if (error) {
      return NextResponse.json({ message: 'Gagal membuat payment', error: error.message }, { status: 500 });
    }

    return NextResponse.json({ message: 'Payment berhasil dibuat', data }, { status: 201 });
  } catch (err) {
    if (err instanceof ApiError) return NextResponse.json({ message: err.message }, { status: err.status });
    return NextResponse.json({ message: 'Terjadi kesalahan server', error: err.message }, { status: 500 });
  }
